...
/// poll for something
///
/// Enter `~createPoll option1 option2 etc`
#[poise::command(prefix_command, guild_only)]
pub async fn createPoll(
    ctx: Context<'_>,
    #[rest]
    #[description = "Text to say"]
    msg: String,
) -> Result<(), Error> {
    let mut args = msg.split(' ').collect::<Vec<&str>>();
    // let mut hash_map = ctx.data().poll.lock().unwrap();
    if args.len() < 3 {
        ctx.say("Bad format. please use createPoll <description> <option1> <option2> etc")
            .await?;
        return Ok(());
    }
    ...
    let entry = Poll {
        description: args.remove(0).to_string(),
        options: args.into_iter().map(|i| (i.to_string(), 0)).collect(),
    };

    ctx.data().poll.lock().unwrap().insert(guild_id, entry);

    let response = format!("Successfully voted for votes!",);
...
pub async fn getPoll(ctx: Context<'_>) -> Result<(), Error> {
    let guild_id = ctx.guild_id().unwrap().to_string();

    let return_value = {
        let hash_map = ctx.data().poll.lock().unwrap();
        let poll = hash_map.get(&guild_id);

        if let Some(poll) = poll {
            let current_view = poll
                .options
                .iter()
                .map(|(name, votes)| format!("{}: {}", name, votes))
                .collect::<Vec<String>>()
                .join("\n");
            let current_winning = poll.options.iter().max_by(|x, y| x.1.cmp(&y.1)).unwrap();
            Some(format!(
                "{}\n current winning item is: {} with {} votes \n {}",
                poll.description, current_winning.0, current_winning.1, current_view
            ))
        } else {
            None
        }
    ...
    if let Some(value) = return_value {
        ...
        ctx.say("no currently open poll open poll with createPoll command")
            .await?;
    }
...
pub async fn vote(
    ...
    let return_value = {
        let mut hash_map = ctx.data().poll.lock().unwrap();
        let poll = hash_map.get_mut(&guild_id);

        if let Some(poll) = poll {
            if let Some(index) = poll.options.iter().position(|x| x.0 == choice) {
                poll.options[index].1 += 1;
                Some(poll.options[index].1)
            } else {
                None
            }
        } else {
    ...
    if let Some(value) = return_value {
        ...
        ctx.say(format!(
            "{} is not avaliable create a poll with that option",
            choice
        ))
        .await?;
    }
...
pub async fn endPoll(ctx: Context<'_>) -> Result<(), Error> {
    let guild_id = ctx.guild_id().unwrap().to_string();
    ...
    if return_value.is_some() {
        ctx.say("Successfully removed poll").await?;
    } else {
        ctx.say("no poll to remove").await?;
    }
